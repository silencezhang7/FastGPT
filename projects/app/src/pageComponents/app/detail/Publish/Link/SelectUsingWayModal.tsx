...
const SelectUsingWayModal = ({ share, onClose }: { share: OutLinkSchema; onClose: () => void }) => {
  ...
  const nginxURL = process.env.NGINX_URL ?? 'http://10.138.93.58:8080';
  ...
  const authLinkUrl = `${nginxURL}/chat/share?shareId=${share?.shareId}${
    getValues('showHistory') ? '' : '&showHistory=0'
  }`;
  ...
  const wayMap = {
    [UsingWayEnum.link]: {
      ...
      code: getValues('enableAuth') ? authLinkUrl : linkUrl // 根据开关状态选择URL
    },
    [UsingWayEnum.iframe]: {
      blockTitle: t('common:core.app.outLink.Iframe block title'),
      code: `<iframe
  src="${getValues('enableAuth') ? authLinkUrl : linkUrl}" // 根据开关状态选择URL
...
/>`
  ...
  id="chatbot-iframe" 
  data-bot-src="${getValues('enableAuth') ? authLinkUrl : linkUrl}" // 根据开关状态选择URL
